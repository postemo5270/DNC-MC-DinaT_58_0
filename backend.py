...
# Tabla 310.16 (Anterior 310.15(B)(16))
TABLA_310_16_CU = { 
...
# ALUMINIO: Nota que NO tiene calibre 14. Empieza en 12.
TABLA_310_16_AL = { 
...
class Circuito:
    ...
    def ejecutar_seleccion_conductor(self):
        ...
        f_temp = 1.0
        # Tabla simplificada basada en 30C (estándar NEC viejo) o 310.15(B)
        # Ajustamos para que coincida con la lógica general:
        if 26 <= t <= 30: f_temp = 1.0
        elif 31 <= t <= 35: f_temp = 0.96
        elif 36 <= t <= 40: f_temp = 0.91
        elif 41 <= t <= 45: f_temp = 0.87
        elif 46 <= t <= 50: f_temp = 0.82
        elif 51 <= t <= 60: f_temp = 0.71 # Simplificado
        elif t > 60: f_temp = 0.41
        ...
        while not encontrado:
            ...
            # --- PROTECCIÓN: Verificar si el calibre existe en la tabla seleccionada ---
            # (El Aluminio no tiene calibre 14, así que debemos saltarlo)
            if cal not in tabla_amp:
                idx += 1
                if idx >= len(ORDEN_CALIBRES): # Evitar loop infinito si se acaban
                     n_cond += 1; idx = 0
                     if n_cond > 6: break 
                continue
            # -------------------------------------------------------------------------
        ...
        self._res_conductor = {
            ...
            "I_Diseno": i_diseno,
            "Calibre": cal_elegido if cal_elegido else "?", 
            "N_Hilos": n_cond, 
            "Tierra": gnd,
            "Amp_Real": amp_real, 
            "V_Caida": v_caida, 
